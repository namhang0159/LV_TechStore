                ...
                <div className="absolute right-4 top-1/2 -translate-y-1/2 flex flex-col gap-3">
                  {productImages.map((_, idx) => (
                    <button
                      ...
                      className={`w-3 h-3 rounded-full transition-colors ${idx === currentImageIndex
                        ? "bg-blue-600"
              ...
              <div className="flex justify-center gap-3 overflow-x-auto p-2">
                {productImages.map((img, idx) => (
                  <button
                    ...
                    type="button"
                    onClick={() => onImageSelect(idx)}
                    className={`w-16 h-16 border-2 transition-colors rounded overflow-hidden flex-shrink-0 ${idx === currentImageIndex
                      ? "border-blue-600"
                    ...
                    <div className="flex flex-wrap gap-2">
                      {attr.values.map(val => (
                        <button
                          ...
                          className={`px-4 py-2 border rounded-md text-sm font-medium transition-colors ${selectedOptions[attr.id] === val.id
