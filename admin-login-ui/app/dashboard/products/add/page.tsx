  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        name: baseInfo.name,
        slug: baseInfo.slug,
        brand_id: parseInt(baseInfo.brand_id, 10),
        category_id: parseInt(baseInfo.category_id, 10),
        base_price: parseFloat(baseInfo.base_price),
        is_featured: isFeatured,
        is_best_seller: isBestSeller,
        warranty_months: parseInt(baseInfo.warranty_months, 10),
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-12">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden p-6 space-y-4">
          <h2 className="text-lg font-semibold text-slate-800 border-b border-slate-100 pb-2">Rich Description</h2>
          {descriptions.map((desc, idx) => (
            <div key={idx} className="space-y-2 pb-12">
